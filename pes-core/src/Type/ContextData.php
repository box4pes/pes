...
class ContextData extends \ArrayObject implements ContextDataInterface {
     ...
     * Užití dat lze zapsat do logu pomocí specializovaného objektu Pes\Core\Type\ContextDataUsage.
     *
    ...
    public function offsetGet(mixed $key): mixed {
        $g = parent::offsetGet($key);
        if ($this->debugMode) {
            if (isset($g)) {
                $this->contextStatus[$key][] = self::GET_EXISTING_VALUE;
            } else {
                $this->contextStatus[$key][] = self::GET_NONEXISTING_VALUE;
            }
        }
        return $g;
    }
