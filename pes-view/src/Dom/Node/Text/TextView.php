...
namespace Pes\View\Dom\Node\Text;
...
class TextView extends TextAbstract implements TextViewInterface {
     ...
     * V této metodě je volána metoda render() zadaného view a jsou jí předána data zadaná jako parametr metody ($view->render($data)).
     * View objekt tedy generuje textový obsah s použitím dat zadaných metodou setData() nebo dat zadaných při volání této matody. To umožňuje použití, kdy data
     * jsou známa až v okamžiku volání getText().
     *
     * @param array $data
     * @return string
     */
    public function getText($data=[]) {
        return $this->view->getString($data);
    }
}
